...
cargo test -p simulator test_network_sync_sim || { echo -e "${RED}Si test failed${NC}"; exit 1; }
cargo test -p simulator epoch || { echo -e "${RED}Simulator Epoch test failed${NC}"; exit 1; }
