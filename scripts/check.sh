...
cargo test -p dag --features simulator test_network_sync_sim || { echo -e "${RED}Simulator test failed${NC}"; exit 1; }
cargo test -p dag --features simulator epoch || { echo -e "${RED}Simulator Epoch test failed${NC}"; exit 1; }
